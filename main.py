...
# JARVIS AI SOC assistant -- local Ollama/Qwen tool-calling loop (chat +
# navigation + read/write SIEM tools). See assistant.py for the full
# design. REPLACES the old DashScope-hosted /assistant/chat + /assistant/
# context endpoints that used to live directly in this file (see git
# history) -- those were read-only advisory only and had no navigation
# or action tools; this router supersedes them entirely under the same
# /assistant/chat path, now running against a local model instead of a
# hosted API key.
